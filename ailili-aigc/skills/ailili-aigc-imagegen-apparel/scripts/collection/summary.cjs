"use strict";

const fs = require("node:fs");
const path = require("node:path");
const { taskResultPath } = require("./one-task.cjs");
const { TYPE_LABELS } = require("../prompts.cjs");

function readTaskResults(datadir, expectedIds) {
  return expectedIds.map((tid) => {
    const file = taskResultPath(datadir, tid);
    if (!fs.existsSync(file)) return { id: tid, status: "pending", error: "task-result 尚未写出" };
    try {
      return JSON.parse(fs.readFileSync(file, "utf8"));
    } catch (error) {
      return { id: tid, status: "failed", error: `task-result 读取失败: ${error.message}` };
    }
  });
}

function toChatPath(filePath) {
  return String(filePath || "").replace(/\\/g, "/");
}

function imageMarkdown(label, caption, filePath) {
  const src = toChatPath(filePath);
  if (!src) return "";
  const alt = [label, caption].filter(Boolean).join(" · ") || "image";
  return `![${alt}](${src})`;
}

function collectionStatus(state) {
  const datadir = path.resolve(state.datadir);
  const specs = state.task_specs || [];
  const results = readTaskResults(
    datadir,
    specs.map((s) => s.id)
  );
  let pending = 0;
  let completed = 0;
  let failed = 0;
  for (const row of results) {
    if (row.status === "success" || row.status === "dry-run") completed += 1;
    else if (row.status === "failed") failed += 1;
    else pending += 1;
  }
  return {
    status: pending > 0 ? "running" : "dispatch_complete",
    command: "collection status",
    pending,
    completed,
    failed,
    total: specs.length,
    done: pending === 0,
    all_ok: pending === 0 && failed === 0,
    tasks: results.map((row, i) => {
      const spec = specs[i] || {};
      const label = spec.label || row.label || TYPE_LABELS[spec.type] || "";
      const point = row.point || spec.point || "";
      const images = row.images || [];
      const markdown =
        (row.status === "success" || row.status === "dry-run") && images[0]
          ? [`### ${label}`, point ? `卖点：${point}` : "", imageMarkdown(label, point, images[0])]
              .filter(Boolean)
              .join("\n")
          : "";
      return {
        id: row.id,
        type: spec.type,
        label,
        status: row.status,
        images,
        point,
        markdown,
        error: row.error || null,
      };
    }),
  };
}

function runSummaryPhase(state) {
  const specs = state.task_specs || [];
  const results = readTaskResults(
    path.resolve(state.datadir),
    specs.map((s) => s.id)
  );
  const pending = results.filter((r) => r.status === "pending").length;
  if (pending > 0) {
    throw new Error(`还有 ${pending} 个任务未完成，请先 --phase status 等到 done=true 再 summary`);
  }
  const total = specs.length;
  const successCount = results.filter((r) => r.status === "success" || r.status === "dry-run").length;
  const lines = [];
  if (total === successCount) lines.push(`**${successCount}/${total} 全部生成成功**`);
  else if (successCount === 0) lines.push(`**0/${total} 生成失败**`);
  else lines.push(`**${successCount}/${total} 生成成功，${total - successCount} 张失败**`);
  const failedRows = results
    .map((result, i) => ({ result, i }))
    .filter(({ result }) => result.status === "failed");
  if (failedRows.length) {
    lines.push("", "**失败项：**", "");
    for (const { result, i } of failedRows) {
      const spec = specs[i] || {};
      lines.push(`- 第 ${i + 1} 张 · ${spec.label || spec.type}：${String(result.error || "未知错误").split("\n")[0]}`);
    }
  }
  process.stdout.write(`${lines.join("\n")}\n`);
  process.stdout.write(
    `${JSON.stringify({ status: "summary", total, completed: successCount, failed: total - successCount, done: true })}\n`
  );
}

module.exports = { collectionStatus, runSummaryPhase };
