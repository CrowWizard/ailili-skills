...
pub const DAEMON_HOST_ENV: &str = "AILILI_AIGC_DAEMON_HOST";
pub const DAEMON_PORT_ENV: &str = "AILILI_AIGC_DAEMON_PORT";
const DEFAULT_HOST: &str = ailili_aigc_server::DEFAULT_HOST;
const DEFAULT_PORT: u16 = ailili_aigc_server::DEFAULT_PORT;
        ...
        other => {
            let payload = json!({
                ...
                "error": {
                    ...
                    "detail": { "usage": "ailili-aigc daemon [start|stop|status|foreground]" }
                }
            });
            println!(
                "{}",
...
pub fn run_foreground() -> i32 {
    ...
    match ailili_aigc_server::run_api_only(host, port) {
        Ok(()) => 0,
        Err(error) => {
            let payload = json!({
                "ok": false,
                "error": { "code": "daemon_failed", "message": error.to_string() }
            });
            eprintln!(
                "{}",
                serde_json::to_string_pretty(&payload).unwrap_or_default()
...
fn acquire_start_lock() -> StartLock {
    let path = daemon_home().join("daemon.start.lock");
...
fn spawn_windows_via_wmi(exe: &Path, log_path: &Path) -> Result<(), String> {
    ...
    if wmi_create_process_cim(&command_line).is_ok() {
        return Ok(());
    }
    wmi_create_process_wmic(&command_line)
}

#[cfg(windows)]
fn write_windows_launcher(exe: &Path, log_path: &Path) -> Result<PathBuf, String> {
    let launcher = daemon_home().join("daemon-launch.ps1");
    ...
    body.push_str("$env:AILILI_AIGC_DAEMON = '1'\n");
    body.push_str(&format!(
        "& {} daemon foreground *>> {}\n",
